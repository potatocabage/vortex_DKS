...
module VX_kmu_refactored_dcr_kd import VX_gpu_pkg::*; (
    ...
    always_ff @(posedge clk) begin
        ...
        if (reset) begin
            ...
            `TRACE(1,  ("%t: KMU_KD: Handshake valid: hwq_data: PC: %h, grid_dim: %d, %d, %d, block_dim: %d, %d, %d, param: %h\n",
               $time, hwq_data.pc, hwq_data.grid_dim[0], hwq_data.grid_dim[1], hwq_data.grid_dim[2],
                hwq_data.block_dim[0], hwq_data.block_dim[1], hwq_data.block_dim[2], hwq_data.param))
            counter_x    <= 0;
            counter_y    <= 0;
            counter_z    <= 0;
            counter_id   <= 0;
            ...
            `TRACE(1,  ("%t: KMU_KD: entered all_cta_sent\n", $time))
            kmu_kd_ready_reg <= 1'b1; // ready for next data, skip current cycle
            kmu_data_valid <= 1'b0;
        end else begin
            `TRACE(1,  ("%t: KMU_KD: entered else, all_cta_sent: %b, kmu_data_valid: %b, kmu_bus_out[0].req_ready: %b\n",
            $time, all_cta_sent, kmu_data_valid, kmu_bus_out[0].req_ready))
            ...
            if (all_cta_sent) begin
                ...
                if (!kmu_bus_out[0].req_valid) begin
                    ...
                    `TRACE(1,  ("%t: KMU_KD: prepared and sent one cta: counter (%d, %d, %d), counter_id: %d\n",
                ...
                if (kmu_bus_out[0].req_valid && kmu_bus_out[0].req_ready) begin
                    // Advance counters
                     `TRACE(1,  ("%t: KMU_KD: counters advanced\n", $time))
