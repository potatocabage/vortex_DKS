...
module VX_kmu import VX_gpu_pkg::*; (
    ...
    // HWQ write request
    input  wire                             hwq_wr_valid,
    input  wire [VX_DCR_ADDR_WIDTH-1:0]     hwq_wr_addr,
    input  wire [VX_DCR_DATA_WIDTH-1:0]     hwq_wr_data,
    ...
    always_ff @(posedge clk) begin
        ...
        //     smem_size          <= '0;
        // end else if (dcr_wr_valid) begin
        if (hwq_wr_valid) begin
            case(hwq_wr_addr)
                // PC
                `VX_HWQ_BASE_STARTUP_ADDR0: kmu_data.pc <= hwq_wr_data;
                // PARAM
                `VX_HWQ_BASE_STARTUP_ARG0:  kmu_data.param <= hwq_wr_data;
                // Grid_dim
                `VX_HWQ_BASE_GRID_DIM0:     kmu_data.grid_dim[0] <= hwq_wr_data;
                `VX_HWQ_BASE_GRID_DIM1:     kmu_data.grid_dim[1] <= hwq_wr_data;
                `VX_HWQ_BASE_GRID_DIM2:     kmu_data.grid_dim[2] <= hwq_wr_data;
                // Block_dim
                `VX_HWQ_BASE_BLOCK_DIM0:    kmu_data.block_dim[0] <= hwq_wr_data;
                `VX_HWQ_BASE_BLOCK_DIM1:    kmu_data.block_dim[1] <= hwq_wr_data;
                `VX_HWQ_BASE_BLOCK_DIM2:    kmu_data.block_dim[2] <= hwq_wr_data;
                // Shared memory size
                `VX_HWQ_BASE_SMEM_SIZE:     smem_size <= hwq_wr_data;
                // `VX_HWQ_BASE_START_EXE:     start <= '1;
                default: ; // ignore
            endcase
        end
    end
