...
public:
  ...
  static int generate() {
    return rand() % 100;  // Small values for easier verification
  }
...
public:
  ...
  static float generate() {
    return static_cast<float>(rand() % 100);  // Small integer values as floats
  }
...
static void show_usage() {
   std::cout << "Vortex Double Vector Add with Dynamic Kernel Launch Test." << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  RT_CHECK(vx_mem_alloc(device, buf_size, VX_MEM_READ, &buf_A));
  RT_CHECK(vx_mem_address(buf_A, &kernel_arg.buf_A));
  
  RT_CHECK(vx_mem_alloc(device, buf_size, VX_MEM_READ, &buf_B));
  RT_CHECK(vx_mem_address(buf_B, &kernel_arg.buf_B));
  
  RT_CHECK(vx_mem_alloc(device, buf_size, VX_MEM_READ_WRITE, &buf_C));
  ...
  std::cout << "initialize vectors A and B" << std::endl;
  ...
  std::cout << "upload vector A" << std::endl;
  RT_CHECK(vx_copy_to_dev(buf_A, h_A.data(), 0, buf_size));

  std::cout << "upload vector B" << std::endl;
  RT_CHECK(vx_copy_to_dev(buf_B, h_B.data(), 0, buf_size));
  ...
  RT_CHECK(vx_copy_from_dev(h_C.data(), buf_C, 0, buf_size));

  // Verify result: C[i] should equal A[i] + B[i] + B[i] = A[i] + 2*B[i]
  std::cout << "verify result" << std::endl;
  int errors = 0;
  for (uint32_t i = 0; i < size; ++i) {
    TYPE expected = h_A[i] + h_B[i] + h_B[i];  // A + B (parent) + B (child)
